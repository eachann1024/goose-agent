...
/// One live attach: a PTY channel running `gooseagent … attach` on the device,
...
/// gooseagent encodes them properly server-side.
@MainActor
final class MobileAttachSession: ObservableObject {
    ...
    /// The gooseagent pane behind this attach, for key/prompt RPCs.
    let paneID: String
    ...
    private func ingest(_ data: Data) {
        ...
        guard let range = bootstrapBuffer.firstRange(of: MobileAttach.bootstrapMarker) else {
            // Cap the gate so a gooseagent that never prints the marker (old
    ...
    /// Sends named keys through gooseagent's RPC — proper terminal encoding without
    ...
    /// Sends a prompt to the agent; gooseagent delivers and submits it.
