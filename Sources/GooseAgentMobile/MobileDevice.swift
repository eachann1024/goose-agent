...
/// A Mac (or other host) running gooseagent, reached over SSH or through the
/// gooseagent.tailcat plugin's tunnel. Unlike the Mac app there is no "Local"
/// device — the phone always talks to a remote gooseagent. Secrets never live
...
struct MobileDevice: Codable, Identifiable, Hashable {
    enum Kind: String, Codable, CaseIterable {
        case ssh
        /// Control plane only on iOS: the tunnel carries the gooseagent API and
    ...
    /// Socket path override; nil means ~/.config/gooseagent/gooseagent.sock on the host.
...
enum MobileSecretStore {
    private static let service = "dev.eachann.gooseagent.ios.ssh"
