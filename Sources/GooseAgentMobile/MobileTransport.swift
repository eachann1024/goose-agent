...
/// How the phone reaches a device's gooseagent session: direct SSH with a
/// `direct-streamlocal` channel per RPC (gooseagent is one-request-per-connection),
...
protocol MobileTransport: AnyObject, Sendable {
    ...
    func events(kinds: [String], statusPaneIDs: [String]) -> AsyncThrowingStream<GooseAgentEvent, Error>
...
extension MobileTransport {
    func request<T: Decodable>(
        ...
        do {
            ...
            throw GooseAgentError.malformedResponse("\(method): \(error)")
        }
    }
}
...
enum MobileTransportError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .tailcatTerminalUnsupported:
            return String(localized: "Terminal attach isn't available over tailcat on iOS yet — the tunnel carries Goose Agent's control plane only.")
        }
    }
}

/// Owns one authenticated SSH connection to a device. RPCs each open a fresh
/// streamlocal channel (gooseagent closes the socket after one reply); the event
...
final class SSHDirectTransport: MobileTransport {
    ...
    /// remote gooseagent socket path against the remote $HOME.
    static func connect(device: MobileDevice) async throws -> SSHDirectTransport {
        ...
        do {
            ...
            if let override = device.socketPath, !override.isEmpty {
                ...
                socketPath = home + "/.config/gooseagent/gooseagent.sock"
            }
    ...
    func request(method: String, params: JSONValue) async throws -> JSONValue {
        ...
        do {
            ...
            throw GooseAgentError.remoteGooseAgentDown(target: "device", socketPath: socketPath)
        }
    ...
    ) -> AsyncThrowingStream<GooseAgentEvent, Error> {
                    ...
                    subscriptionAttempt: while !Task.isCancelled {
                        ...
                        do {
                            ...
                            eventRead: while !Task.isCancelled {
                                // Long timeout: gooseagent only writes when something happens.
                                ...
                                while let index = buffer.firstIndex(of: 0x0A) {
                                    ...
                                    guard sawAck else {
                                        ...
                                        sawAck = true
                                        continuation.yield(GooseAgentEvent(
                                            kind: GooseAgentEvent.subscriptionStartedKind,
                                            payload: .object([:])
                                        ))
                                        continue
                                    }
                        ...
                        if Task.isCancelled {
                            ...
                            continuation.finish(throwing: GooseAgentError.connectionFailed(
                                "event stream ended"
                            ))
                        }
                        return
...
/// tailcat control-plane transport: the embedded bridge (GooseTailcat, via
/// GooseKit's `TailcatBridgeManager`) holds the WireGuard/DERP session and
/// re-serves the remote gooseagent API socket on a local Unix socket, so RPC and
...
final class TailcatMobileTransport: MobileTransport {
    ...
    func request(method: String, params: JSONValue) async throws -> JSONValue {
        do {
            ...
            // actionable version, mirroring GooseService's tailcat path.
            if let detail = await TailcatBridgeManager.shared.recentError(deviceID: deviceID) {
                throw GooseAgentError.tailcatBridgeFailed(detail)
            }
            throw error
        }
    }
...
enum MobileAttach {
    /// Non-login sshd exec leaves PATH at /usr/bin:/bin; gooseagent usually lives in
    ...
    static let bootstrapMarker = Data([0x1B, 0x5F]) + Data("gooseagent-attach".utf8) + Data([0x1B, 0x5C])
    private static let markerPrintf = #"printf '\033_gooseagent-attach\033\\'"#
    ...
    static func command(target: TerminalAttachTarget) -> String {
        ...
        case .agent(let paneID):
            attach = "gooseagent agent attach \(ShellQuoting.quoted(paneID)) --takeover"
        case .terminal(let terminalID):
            attach = "gooseagent terminal attach \(ShellQuoting.quoted(terminalID)) --takeover"
        }
