...
/// One device's live gooseagent view: transport + snapshot + event pump.
@MainActor
final class MobileDeviceSession {
            ...
            do {
                eventSubscriptions: while !Task.isCancelled {
                    ...
                    let stream = transport.events(
                        kinds: GooseAgentEvent.allKinds,
                    ...
                    for try await event in stream {
                        ...
                        if event.kind == GooseAgentEvent.agentStatusChangedKind,
                           self.applyAgentStatusEvent(event) {
                            ...
                            || event.kind == GooseAgentEvent.agentStatusChangedKind
    ...
    private func applyAgentStatusEvent(_ event: GooseAgentEvent) -> Bool {
...
final class MobileAppModel {
    ...
    /// Bare gooseagent terminal panes in the selected space (nil = all).
