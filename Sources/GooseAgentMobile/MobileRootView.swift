        ...
        NavigationSplitView {
            ...
            if let device = model.selectedDevice, device.isTailcat,
               model.selectedAgentPaneID != nil {
                // tailcat carries gooseagent's control plane only — no shell, no
                ...
                ContentUnavailableView(
                    ...
                    description: Text(String(localized: "This device's tunnel reaches Goose Agent's control plane only. Use an SSH device to attach to a live terminal."))
                )
        ...
        List(selection: $model.selectedAgentPaneID) {
            if model.devices.isEmpty {
                ContentUnavailableView {
                    ...
                    Text(String(localized: "Add a Mac running Goose Agent to get started."))
                } actions: {
            ...
            Form {
                ...
                if kind == .tailcat {
                    Section {
                        ...
                        Text(String(localized: "From the host: gooseagent plugin action invoke gooseagent.tailcat.token — the token is stored in this device's Keychain, and the tunnel needs no SSH."))
                    }
                } else {
                    sshSections
                }
    ...
    private var sshSections: some View {
        ...
        if authMethod == .deviceKey {
            Section(String(localized: "This Phone's Key")) {
                ...
                Text(String(localized: "On the Mac, run: echo '<key>' >> ~/.ssh/authorized_keys — or paste the line into Goose Agent's upcoming pairing screen."))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        } else {
