...
/// A local child process attached to a pseudo-terminal — the byte transport
/// for a host-managed Ghostty surface.
///
...
final class TerminalProcess: @unchecked Sendable {
    ...
    private let ioQueue = DispatchQueue(label: "dev.eachann.gooseagent.terminal-process.io")
    ...
    private let writeQueue = DispatchQueue(label: "dev.eachann.gooseagent.terminal-process.write")
    private let stateLock = NSLock()
    private var exitReported = false

    /// Called on the IO queue with each chunk read from the PTY.
    var onOutput: ((Data) -> Void)?
    /// Called on the IO queue once the PTY reaches EOF, after its last output
    /// chunk. The child exit notification may arrive before or after this.
    var onOutputEnd: (() -> Void)?
    ...
    /// Report the child immediately; trailing PTY output may still be in flight.
    /// The EOF callback runs separately after those last bytes have been read.
