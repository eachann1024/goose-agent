...
/// the host's gooseagent.tailcat plugin and serves it on a local Unix socket, so
/// `SocketRPC`, the event stream, and (on macOS) terminal attach reach the
/// remote gooseagent exactly as the SSH forward lets them — by talking to a local
...
/// and owns cleanup. It is deliberately free of GooseKit types so GooseKit can
...
public actor TailcatBridge {
    ...
    public nonisolated static func localSocketPath(deviceID: UUID) -> String {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("gooseagent-tailcat", isDirectory: true)
