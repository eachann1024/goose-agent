// Package bridge is the shared core of gooseagent's tailcat client transport. It
// holds one tailcat (WireGuard/DERP) session to a host exposing its gooseagent
// socket through the gooseagent.tailcat plugin, and serves that socket on local
// Unix sockets so gooseagent clients reach the remote gooseagent exactly as if it were
// local.
//
// Two local sockets are served, mirroring the two a gooseagent server listens on:
...
// forwarded to the plugin's client port). Terminal attach (`gooseagent agent
// attach`) derives the client socket from GOOSEAGENT_SOCKET_PATH, so without the
// second listener every attach dies with ENOENT.
//
// The package is consumed two ways: the `gooseagent-tailcat-bridge` CLI wraps it in
...
// Default tunnel ports the gooseagent.tailcat plugin serves on. Control carries the
...
// DeriveClientSocket mirrors gooseagent's derive_client_socket_from_api_socket:
// insert "-client" before the ".sock" extension (gooseagent.sock -> gooseagent-client.sock).
