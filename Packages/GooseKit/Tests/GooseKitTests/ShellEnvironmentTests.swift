...
final class ShellEnvironmentTests: XCTestCase {
    ...
    func testCursorKindMapsToCursorAgent() {
        XCTAssertEqual(GooseService.binaryName(for: "cursor"), "cursor-agent")
        XCTAssertEqual(GooseService.binaryName(for: "codex"), "codex")
    }
    ...
    func testInstalledAgentsHonorsOverrideAndCursorAlias() async throws {
        ...
        let service = GooseService(device: .local, localServer: nil)
    ...
    func testCaptureRunsLoginAndInteractiveStartupFilesAndIgnoresStdoutBanners() throws {
        ...
        try "export GOOSEAGENT_LOGIN_MARKER=from-zprofile\n"
            .write(to: directory.appendingPathComponent(".zprofile"), atomically: true, encoding: .utf8)
        try """
        echo 'WELCOME TO ZSH'
        print -P '%F{red}prompt%f'
        export GOOSEAGENT_INTERACTIVE_MARKER=from-zshrc
        export GOOSEAGENT_COMPLEX_VALUE='left=right
        second line'
        ...
        XCTAssertEqual(snapshot["GOOSEAGENT_LOGIN_MARKER"], "from-zprofile")
        XCTAssertEqual(snapshot["GOOSEAGENT_INTERACTIVE_MARKER"], "from-zshrc")
        XCTAssertEqual(snapshot["GOOSEAGENT_COMPLEX_VALUE"], "left=right\nsecond line")
    ...
    func testInteractiveHangFallsBackToLoginShell() throws {
        ...
        XCTAssertEqual(snapshot["GOOSEAGENT_LOGIN_MARKER"], "from-zprofile")
        XCTAssertNil(snapshot["GOOSEAGENT_INTERACTIVE_MARKER"], "killed -i must not leak a partial interactive rc")
