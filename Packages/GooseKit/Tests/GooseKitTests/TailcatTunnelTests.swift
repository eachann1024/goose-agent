...
final class TailcatBridgeTests: XCTestCase {
    ...
    /// real token and ping the remote gooseagent over its local socket, proving the
    /// WireGuard/DERP path end to end with no external `tailcat` CLI. Needs a
    /// live token: GOOSEAGENT_E2E_TAILCAT_TOKEN=$(gooseagent plugin action … token).
    func testEmbeddedBridgeRoundtripsAPing() async throws {
        guard let token = ProcessInfo.processInfo.environment["GOOSEAGENT_E2E_TAILCAT_TOKEN"],
              !token.isEmpty
        else {
            throw XCTSkip("set GOOSEAGENT_E2E_TAILCAT_TOKEN to run the live embedded-tailcat E2E")
        }
        let id = UUID()
        defer {
            TailcatCredentialStore.removeToken(for: id)
    ...
    func testEnsureUpWithoutATokenThrowsTokenMissing() async {
        ...
        do {
            ...
            XCTFail("expected GooseAgentError.tailcatTokenMissing, got \(error)")
        }
    }
}
