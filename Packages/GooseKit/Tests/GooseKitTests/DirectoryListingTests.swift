...
final class DirectoryListingTests: XCTestCase {
    ...
    private func makeLocalService() -> GooseService {
        GooseService(device: Device(name: "Test", kind: .local), localServer: nil)
    }
    ...
    func testShellQuotedSurvivesTheAwkwardCharacters() {
        XCTAssertEqual(GooseService.shellQuoted("/plain/path"), "'/plain/path'")
        XCTAssertEqual(GooseService.shellQuoted("/it's here"), "'/it'\\''s here'")
        // Single quotes keep $HOME and backticks literal; no escaping needed inside.
        XCTAssertEqual(GooseService.shellQuoted("/a $HOME `b`"), "'/a $HOME `b`'")
    }
}
