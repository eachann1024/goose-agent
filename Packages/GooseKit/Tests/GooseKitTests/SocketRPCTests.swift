...
final class SocketRPCTests: XCTestCase {
    ...
    func testDecodeEventNormalizesLifecycleAndScopedEventNames() throws {
        ...
        XCTAssertEqual(status.kind, GooseAgentEvent.agentStatusChangedKind)
        ...
        XCTAssertEqual(snakeStatus.kind, GooseAgentEvent.agentStatusChangedKind)
    }

    func testStaleStatusPaneRetriesWithoutScopedSubscriptions() {
        XCTAssertTrue(SocketRPC.shouldRetryStatusSubscription(
            after: GooseAgentError.rpc(code: "pane_not_found", message: "gone"),
            statusPaneIDs: ["w1:p1"]
        ))
        ...
        XCTAssertFalse(SocketRPC.shouldRetryStatusSubscription(
            after: GooseAgentError.rpc(code: "invalid_params", message: "bad"),
            statusPaneIDs: ["w1:p1"]
        ))
    }
