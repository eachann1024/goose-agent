...
final class SSHAuthenticationTests: XCTestCase {
    func testForwardingFailureExtractsTheLastOpenSSHChannelError() {
        ...
        channel 2: open failed: connect failed: dial unix /tmp/gooseagent.sock: connect: connection refused
        """
    ...
    func testLegacyLocalCredentialMigratesToKeychain() throws {
        ...
        let query: [String: Any] = [
            ...
            kSecAttrService as String: "dev.eachann.gooseagent.ssh-password",
        ...
        let passwordDirectory = applicationSupport
            .appendingPathComponent("GooseAgent/SSHCredentials/passwords", isDirectory: true)
...
final class AttachBinarySelectionTests: XCTestCase {
    ...
    func testKnownServerVersionProbesForAnExactMatch() {
        let fragment = GooseService.attachBinarySelection(serverVersion: "0.8.2")
        XCTAssertTrue(fragment.contains("for d in $PATH"))
        XCTAssertTrue(fragment.contains("'0.8.2'"))
        XCTAssertTrue(fragment.contains("hb=gooseagent"), "first-found binary must stay the fallback")
    }

    func testUnknownOrUnsafeServerVersionFallsBackToPlainGooseAgent() {
        XCTAssertEqual(GooseService.attachBinarySelection(serverVersion: nil), "hb=gooseagent")
        XCTAssertEqual(GooseService.attachBinarySelection(serverVersion: ""), "hb=gooseagent")
        // Anything but digits and dots must not reach the shell.
        XCTAssertEqual(GooseService.attachBinarySelection(serverVersion: "0.8'; rm -rf /"), "hb=gooseagent")
    }

    func testAttachCommandsRunTheSelectedBinaryThroughSh() {
        let local = GooseService(device: Device(name: "L", kind: .local), localServer: nil)
        ...
        XCTAssertNil(local.environment["TERM"], "the terminal host owns TERM")

        let remote = GooseService(device: Device(name: "R", kind: .ssh(target: "u@h")), localServer: nil)
            .attachCommand(paneID: "w1:p1", serverVersion: "0.8.2")
        XCTAssertEqual(remote.executable, "/usr/bin/ssh")
...
final class SSHFileTransferTests: XCTestCase {
    func testUploadStreamsFileAndReturnsRemotePath() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("gooseagent-upload-test-\(UUID().uuidString)", isDirectory: true)
        ...
        let script = """
        #!/bin/sh
        printf '%s\\n' "$@" > \(GooseService.shellQuoted(argumentsURL.path))
        cat > \(GooseService.shellQuoted(capturedURL.path))
        printf '/home/test/.cache/gooseagent/attachments/test.png\\n'
        """
        ...
        XCTAssertEqual(remotePath, "/home/test/.cache/gooseagent/attachments/test.png")
    ...
    func testUploadRejectsOversizedAndIrregularFiles() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("gooseagent-upload-guard-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
...
final class AgentAttachmentDeliveryPolicyTests: XCTestCase {
    ...
    /// gooseagent 0.8.2's `server.agent_manifests` reports only an id and version —
    ...
    func testBuiltInFallbackCoversVerifiedAgentKinds() throws {
        ...
        // gooseagent's own manifest aliases, in case detection ever reports one.
        ...
        // Advertised by gooseagent but unverified here: no capabilities, plain paste.
