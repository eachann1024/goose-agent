...
/// Per-device facade over gooseagent's socket API. For SSH devices it owns the tunnel.
public actor GooseService {
    ...
    private let localServer: LocalGooseAgentServer?
    ...
    public init(device: Device, autoStartLocalServer: Bool = true) {
        self.init(
            device: device,
            localServer: device.isLocal && autoStartLocalServer ? LocalGooseAgentServer() : nil
        )
    }

    /// Seam for tests: injects the auto-start collaborator, nil turning it off.
    init(device: Device, localServer: LocalGooseAgentServer?) {
        self.device = device
    ...
    public func connect() async throws -> PingResult {
        ...
        case .ssh:
            guard let tunnel else { throw GooseAgentError.tunnelFailed("missing tunnel") }
        ...
        do {
            ...
            if let tunnel {
                if let gooseAgentError = error as? GooseAgentError, Self.isSilentForward(gooseAgentError),
                ...
                if let forwardingFailure = await tunnel.forwardingFailure() {
                    await tunnel.tearDown()
                    throw GooseAgentError.tunnelFailed(forwardingFailure)
                }
            }
            throw error
        }
        guard pong.protocolVersion >= Self.minimumProtocolVersion else {
            throw GooseAgentError.incompatibleProtocol(pong.protocolVersion)
        }
        rpc = client
        everConnected = true
        return pong
    ...
    /// again, so the app boots without the user opening a terminal to run `gooseagent`.
    ///
    /// A server that already answered here and is gone now was stopped deliberately — by
    /// `gooseagent server stop`, or by the restart in the middle of `gooseagent update` — and bringing
    /// it back would both undo the user's decision and let gooseagent win the bind race that
    /// `gooseagent update` needs. The guard is per service instance rather than per process on
    /// purpose: Reconnect and the backoff loop must still be able to start a server for
    /// someone who installed or repaired gooseagent after opening the app.
    private func ping(_ client: SocketRPC, socketPath: String) async throws -> PingResult {
        ...
        } catch let error as GooseAgentError where Self.isServerDown(error) {
    ...
    static func isServerDown(_ error: GooseAgentError) -> Bool {
        switch error {
    ...
    static func isSilentForward(_ error: GooseAgentError) -> Bool {
    ...
    private func client() throws -> SocketRPC {
        guard let rpc else { throw GooseAgentError.connectionFailed("not connected") }
        return rpc
    }
    ...
    public func gitRepositoryInfo(at cwd: String) async -> GitRepositoryInfo? {
        ...
        case .tailcat:
            // tailcat carries the gooseagent socket only; it has no remote shell channel.
            return nil
        }
    }
    ...
    public func homeDirectory() async throws -> String {
        ...
        case .tailcat:
            // The tunnel carries only the gooseagent socket — no shell to probe.
            throw GooseAgentError.fileOperationFailed(
    ...
    /// Creates a workspace (gooseagent "space") rooted at a directory.
    ...
    public func createWorkspace(label: String?, cwd: String?) async throws -> (workspaceID: String, rootPaneID: String?) {
        ...
        else { throw GooseAgentError.malformedResponse("workspace.create returned no workspace_id") }
    ...
    /// Renames an agent (`gooseagent agent rename`). `target` is a pane id or current name.
    ...
    /// Same RPC the gooseagent TUI uses for tab reorder.
    ...
    /// the gooseagent TUI reorders spaces, and it moves a worktree group atomically.
    ...
    public func createTab(workspaceID: String?, cwd: String?, label: String?) async throws -> String {
        ...
        else { throw GooseAgentError.malformedResponse("tab.create returned no root_pane.pane_id") }
        return paneID
    }
    ...
    public func startAgent(
        ...
        while true {
            do {
                ...
                return
            } catch let error as GooseAgentError {
    ...
    static func isPaneBusy(_ error: GooseAgentError) -> Bool {
    ...
    public func readPane(paneID: String) async throws -> (text: String, revision: Int) {
        ...
        guard let text = result["read"]?["text"]?.stringValue else {
            throw GooseAgentError.malformedResponse("pane.read returned no text")
        }
        let revision: Int
    ...
    /// Sends literal text (gooseagent wraps it in bracketed paste when the app enables it —
    ...
    /// terminal key presses by gooseagent.
    ...
    public func stageAttachment(from localURL: URL) async throws -> String {
        ...
        case .ssh:
            guard let tunnel else {
                throw GooseAgentError.fileTransferFailed("no SSH connection for this device")
            }
            return try await tunnel.uploadFile(from: localURL)
        case .tailcat:
            throw GooseAgentError.fileTransferFailed(
    ...
    public func events(statusPaneIDs: [String] = []) throws -> AsyncThrowingStream<GooseAgentEvent, Error> {
    ...
    public nonisolated func terminalCommand(workingDirectory: String? = nil) -> TerminalCommand {
        switch device.kind {
        case .tailcat:
            // The tunnel carries only the gooseagent socket; there is no shell on
            // the other side to run. Standalone terminals need SSH.
            return TerminalCommand(
                executable: "/bin/sh",
                args: ["-c", "echo 'A tailcat device carries only the gooseagent socket — standalone shells need SSH.'; exit 1"],
                environment: [:],
                authorizationID: nil
            )
        case .local:
        ...
        case .ssh(let target):
            ...
            // Same seeding as the remote attach: the terminal child gets a
            // sparse environment, and OpenSSH needs the user's PATH (Match exec)
            // and SSH_AUTH_SOCK (agent identities). Authentication values win.
    ...
    /// Shell fragment that picks the gooseagent binary to attach with. gooseagent's attach
    /// stream requires the CLI and server protocol versions to match exactly, so
    /// when several gooseagent binaries share the PATH (a stale copy in ~/.local/bin
    ...
    /// pick a gooseagent binary whose protocol matches the server's — see
    ...
    public nonisolated func attachCommand(
        ...
        switch device.kind {
        case .local, .tailcat:
            // Same PATH we used to discover `gooseagent`: login-shell snapshot, GUI
            // PATH, well-known prefixes. Discovery and attach must not diverge
            // or a `#!/usr/bin/env node` shim is found and then fails at exec.
            // TERM/COLUMNS/LINES belong to the terminal host.
            //
            // A tailcat device attaches with the LOCAL gooseagent CLI pointed at the
            // tunnel's bridge socket (GOOSEAGENT_SOCKET_PATH is gooseagent's contractual
            ...
            environment.removeValue(forKey: "LINES")
            if device.isTailcat {
                environment["GOOSEAGENT_SOCKET_PATH"] =
                    TailcatBridgeManager.localSocketPath(deviceID: device.id)
            } else if let socketPath = device.socketPath {
                // A named-session Local device: point the local gooseagent CLI at
                // that session's socket (gooseagent's contractual override), or the
                // attach would run against the default session instead.
                environment["GOOSEAGENT_SOCKET_PATH"] = socketPath
            }
