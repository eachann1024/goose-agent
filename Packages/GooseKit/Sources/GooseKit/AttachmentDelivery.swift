...
public struct AgentAttachmentCapabilityRegistry: Sendable, Equatable {
    ...
    /// Compatibility for Goose Agent servers predating manifest capabilities — which
    ...
    /// authoritative. Alias spellings mirror the `aliases` in gooseagent's
    /// agent-detection manifests; gooseagent reports the manifest id as a pane's
