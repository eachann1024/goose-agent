...
/// Discovers gooseagent *named* sessions (`gooseagent --session <name>` / `GOOSEAGENT_SESSION`).
///
/// Each named session is a separate server with its own socket at
/// `~/.config/gooseagent/sessions/<name>/gooseagent.sock` (gooseagent's `data_dir_for`). The
/// default session lives at `~/.config/gooseagent/gooseagent.sock`. gooseagent's Local device
...
public enum GooseSessionDiscovery {
    ...
    public static func namedSessions(
        ...
        let base = configDirectory
            ?? URL(fileURLWithPath: NSHomeDirectory())
                .appendingPathComponent(".config/gooseagent", isDirectory: true)
        ...
        for dir in entries {
            ...
            let socket = dir.appendingPathComponent("gooseagent.sock")
    ...
    static func deterministicID(for name: String) -> UUID {
        ...
        let low = fnv(14_695_981_039_346_656_037, "gooseagent-session:\(name)")
