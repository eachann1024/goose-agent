...
/// Agent status buckets reported by gooseagent snapshots and status events.
...
public struct AgentInfo: Codable, Sendable, Identifiable, Equatable {
    ...
    /// Missing while gooseagent is still detecting the agent in a freshly started pane.
    public let agentKindRaw: String?
    /// gooseagent's agent name (`agent.rename`). Distinct from the kind in `agent`.
    ...
    /// win; then custom gooseagent names; then conversation OSC titles; generic OSC
    /// titles (kind, cwd basename) fall through to the gooseagent name, then kind.
...
public struct WorkspaceInfo: Codable, Sendable, Identifiable, Equatable {
    ...
    public let agentStatusRaw: String?
    /// Root directory when gooseagent reports it on workspace.list / snapshot.
    public let cwd: String?
...
public struct TabInfo: Codable, Sendable, Identifiable, Equatable {
    ...
    /// gooseagent labels fresh tabs with their number ("1", "2"); only a label
...
public struct GooseAgentEvent: Sendable {
...
public enum GooseAgentError: Error, LocalizedError, Sendable {
    case socketUnavailable(String)
    case connectionFailed(String)
    case gooseAgentNotInstalled
    case remoteGooseAgentDown(target: String, socketPath: String)
    ...
    public var errorDescription: String? {
        switch self {
        case .socketUnavailable(let path): return "gooseagent socket not found at \(path)"
        case .connectionFailed(let reason): return "connection failed: \(reason)"
        case .gooseAgentNotInstalled: return "Goose Agent was not found on PATH — install it with \"brew install gooseagent\""
        case .remoteGooseAgentDown(let target, let socketPath):
            return "Goose Agent isn't running on \(target) — nothing listens at \(socketPath); start it by running \"gooseagent\" on that machine"
        case .rpc(let code, let message): return "gooseagent error \(code): \(message)"
        case .malformedResponse(let reason): return "malformed response: \(reason)"
        case .incompatibleProtocol(let version): return "gooseagent protocol \(version) is too old (need >= 17)"
...
/// What an embedded terminal attaches to: an agent pane or a bare gooseagent terminal.
