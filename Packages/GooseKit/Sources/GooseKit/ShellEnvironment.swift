...
///
/// LaunchServices gives gooseagent `/usr/bin:/bin:/usr/sbin:/sbin`. The PATH that actually
...
public struct ShellEnvironment: Sendable, Equatable {
    ...
    /// Directories searched for agent (and gooseagent) binaries, first match wins.
    ...
    static let captureFileKey = "GOOSEAGENT_SHELL_ENV_CAPTURE_FILE"
    ...
    private static func exclusiveCaptureFile(tmpdir: String?) -> String? {
        ...
        for _ in 0..<8 {
            let path = "\(directory).gooseagent-shell-env-\(getpid())-\(UUID().uuidString.prefix(8))"
