...
/// Forwards a remote gooseagent Unix socket to a local one using the system OpenSSH client
...
public actor SSHTunnel {
    ...
    public func probeRemoteHome() async throws -> String {
        ...
        guard home.hasPrefix("/") else {
            throw GooseAgentError.tunnelFailed("could not resolve remote home (got: \(output))")
        }
        remoteHome = home
        return home
    }
    ...
    public func remoteSocketPath() async throws -> String {
        ...
        return "\(home)/.config/gooseagent/gooseagent.sock"
    }
    ...
    public func ensureUp() async throws -> String {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("gooseagent-tunnels", isDirectory: true)
        ...
        for _ in 0..<60 {
            ...
            if !proc.isRunning {
                ...
                throw GooseAgentError.tunnelFailed(Self.failureReason(status: proc.terminationStatus, stderr: errorText))
            }
        ...
        process = nil
        resetErrorCapture()
        throw GooseAgentError.tunnelFailed("timed out waiting for forwarded socket")
    }
    ...
    /// cases apart: no socket file (gooseagent isn't running over there, by far the common
    ...
    public func diagnoseSilentForward() async -> GooseAgentError? {
    ...
    static func silentForwardDiagnosis(
        ...
        case "missing":
            return .remoteGooseAgentDown(target: target, socketPath: remoteSocketPath)
        case "exists":
            return .tunnelFailed(
                "\(remoteSocketPath) exists on \(target) but forwarded connections get no reply"
                    + " — gooseagent may have left a stale socket, or sshd forbids Unix-socket"
    ...
    public static func probeOS(target: String, credentialID: UUID? = nil) async throws -> String {
        ...
        guard !os.isEmpty else { throw GooseAgentError.tunnelFailed("empty OS probe result") }
        return os
    }
    ...
    static func validateUploadCandidate(_ localURL: URL) throws -> Int {
        guard localURL.isFileURL else {
            throw GooseAgentError.fileTransferFailed("a local file is required")
        }
    ...
    static func uploadFile(
        ...
        guard isSafeRemoteFilename(remoteFilename) else {
            throw GooseAgentError.fileTransferFailed("unsafe remote filename")
        }
        let command = """
        umask 077
        dir="${XDG_CACHE_HOME:-$HOME/.cache}/gooseagent/attachments"
        ...
        return try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                do {
                    ...
                    continuation.resume(throwing: GooseAgentError.fileTransferFailed("ssh spawn: \(error.localizedDescription)"))
                    return
                }
                ...
                guard proc.terminationStatus == 0 else {
                    ...
                    continuation.resume(throwing: GooseAgentError.fileTransferFailed(
                ...
                guard let remotePath = lines.last(where: { $0.hasPrefix("/") }) else {
                    continuation.resume(throwing: GooseAgentError.fileTransferFailed("the remote host returned no path"))
                    return
                }
        ...
        try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                do {
                    ...
                    continuation.resume(throwing: GooseAgentError.tunnelFailed("ssh spawn: \(error.localizedDescription)"))
                    return
                }
                ...
                guard proc.terminationStatus == 0 else {
                    let errorData = errorOutput.fileHandleForReading.readDataToEndOfFile()
                    continuation.resume(throwing: GooseAgentError.tunnelFailed(
                        failureReason(status: proc.terminationStatus, stderr: errorData)
                    ))
                    return
                }
