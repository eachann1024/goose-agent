...
///
/// A thin GooseKit-side coordinator over `GooseTailcat.TailcatBridge`: it maps a
...
/// The bridge serves the remote gooseagent on that socket, so `SocketRPC`, the event
...
/// from a fresh `GooseService` (see `AttachTerminalView`) and cannot reach the
