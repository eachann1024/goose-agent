...
/// a bearer credential — anyone holding it can drive the exposed gooseagent — so
/// it never touches devices.json; only the Keychain holds it.
public enum TailcatCredentialStore {
    static let service = "dev.eachann.gooseagent.tailcat-token"
    ...
    public static func token(for deviceID: UUID) throws -> String? {
        ...
        case errSecItemNotFound:
            return nil
        default:
            throw GooseAgentError.tunnelFailed("Keychain read failed (\(status))")
        }
    }
    ...
    public static func setToken(_ token: String, for deviceID: UUID) throws {
        ...
        guard status == errSecSuccess else {
            throw GooseAgentError.tunnelFailed("Keychain write failed (\(status))")
        }
    }
