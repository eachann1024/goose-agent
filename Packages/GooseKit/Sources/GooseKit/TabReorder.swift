...
/// Maps a sidebar drop onto gooseagent's `tab.move` `insert_index`.
///
/// `orderedIDs` is one workspace's tabs in snapshot / `tab.list` display
/// order — not `number`. Goose Agent applies `insert_index` before removing the
...
public enum TabReorder: Sendable {
    ...
    public static func insertIndex(
        ...
        // gooseagent's `move_tab` reads `insert_index` against the list as it is
