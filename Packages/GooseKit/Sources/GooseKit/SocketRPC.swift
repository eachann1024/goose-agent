...
public struct SocketRPC: Sendable {
    ...
    public func events(
        kinds: [String] = GooseAgentEvent.allKinds,
    ...
    public static func eventSubscriptionParams(
        ...
        subscriptions.append(contentsOf: Set(statusPaneIDs).sorted().map {
            JSONValue.object([
                "type": .string(GooseAgentEvent.agentStatusChangedKind),
                "pane_id": .string($0),
            ])
        })
    ...
    public static func decodeEvent(_ line: Data) -> GooseAgentEvent? {
        ...
        let rawKind = value["event"]?.stringValue
            ...
            ?? "unknown"
        return GooseAgentEvent(kind: GooseAgentEvent.normalizedKind(rawKind), payload: value)
    }
    ...
    public static func shouldRetryStatusSubscription(
        ...
        guard !statusPaneIDs.isEmpty,
              case GooseAgentError.rpc(let code, _) = error
        else { return false }
    ...
    public static func encodeRequest(id: String, method: String, params: JSONValue?) -> Data {
        // gooseagent requires `params` to be present even when empty.
    ...
    public static func decodeResponse(_ line: Data?) throws -> JSONValue {
        guard let line, !line.isEmpty else { throw GooseAgentError.malformedResponse("empty reply") }
        let value: JSONValue
        do {
            value = try JSONDecoder().decode(JSONValue.self, from: line)
        } catch {
            throw GooseAgentError.malformedResponse("undecodable reply")
        }
        if let error = value["error"] {
            let code = error["code"]?.stringValue ?? "unknown"
            let message = error["message"]?.stringValue ?? "unknown error"
            throw GooseAgentError.rpc(code: code, message: message)
        }
        guard let result = value["result"] else {
            throw GooseAgentError.malformedResponse("reply has neither result nor error")
        }
        return result
    }
    ...
    static func connect(path: String) throws -> Int32 {
        guard FileManager.default.fileExists(atPath: path) else {
            throw GooseAgentError.socketUnavailable(path)
        }
        let fd = socket(AF_UNIX, SOCK_STREAM, 0)
        guard fd >= 0 else { throw GooseAgentError.connectionFailed("socket(): \(String(cString: strerror(errno)))") }
        ...
        guard setsockopt(
            ...
            close(fd)
            throw GooseAgentError.connectionFailed("setsockopt(SO_NOSIGPIPE): \(reason)")
        }
        ...
        guard rc == 0 else {
            let reason = String(cString: strerror(errno))
            close(fd)
            throw GooseAgentError.connectionFailed("connect(): \(reason)")
        }
        return fd
    }
    ...
    static func writeLine(fd: Int32, data: Data) throws {
        ...
        while !remaining.isEmpty {
            ...
            guard written > 0 else { throw GooseAgentError.connectionFailed("write(): \(String(cString: strerror(errno)))") }
    ...
    static func readLine(fd: Int32, timeoutSeconds: Int32?, buffer: inout Data) throws -> Data? {
        ...
        guard setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tv, socklen_t(MemoryLayout<timeval>.size)) == 0 else {
            throw GooseAgentError.connectionFailed("setsockopt(SO_RCVTIMEO): \(String(cString: strerror(errno)))")
        }
        ...
        while true {
            ...
            if count < 0 {
                ...
                throw GooseAgentError.connectionFailed("read(): \(String(cString: strerror(errno)))")
            }
