...
/// Starts the local gooseagent server on demand, so a cold boot connects instead of telling the
/// user to go run `gooseagent` in a terminal first.
///
...
/// flow is exercisable on a machine where gooseagent isn't installed.
public struct LocalGooseAgentServer: Sendable {
    /// A spawned `gooseagent server`: watched, never owned. See `spawn(binary:)`.
    ...
    /// Grace given to the socket after the spawned process exits. `gooseagent server` also exits
    ...
    public init(
        binaryPath: @escaping @Sendable () -> String? = { LocalGooseAgentServer.resolveBinary() },
        launcher: @escaping @Sendable (String) throws -> Launched = { try LocalGooseAgentServer.spawn(binary: $0) },
    ...
    /// Returns once `socketPath` accepts connections, starting `gooseagent server` if it doesn't.
    ...
    public func ensureRunning(socketPath: String, timeout: TimeInterval = 10) async throws {
        ...
        guard let binary = binaryPath() else { throw GooseAgentError.gooseAgentNotInstalled }
        ...
        var exited = false
        while true {
            // Only a real connection proves the server is up: gooseagent creates the socket file
        ...
        if exited {
            throw GooseAgentError.connectionFailed(
                "gooseagent server exited without serving \(socketPath): \(launched.output())"
            )
        }
        throw GooseAgentError.connectionFailed("timed out waiting for the gooseagent server at \(socketPath)")
    }
    ...
    /// `gooseagent server` reacts to a refused socket by unlinking it and binding its own — which
    /// would strand the running server holding all of the user's panes, unreachable and
    /// invisible to `gooseagent server stop`. A full backlog drains in milliseconds while a stale
    ...
    /// Locates the gooseagent CLI on the same search PATH used for agent CLIs: login-shell
    ...
    public static func resolveBinary(environment: [String: String]? = nil) -> String? {
        ...
        return (ShellEnvironment.cached ?? .empty).findExecutable("gooseagent")
    }

    /// Spawns `gooseagent server` and hands back a handle for watching it.
    ///
    /// The process is deliberately **not** owned: nothing here terminates it, and there is no
    /// counterpart to `SSHTunnel.tearDown()`. gooseagent's server is a shared daemon holding every
    /// agent's PTY, so killing it when gooseagent quits would take the user's running agents down
    /// with it. It is meant to outlive us (launchd reparents it) and a second `gooseagent server`
    ...
    public static func spawn(binary: String, environment: [String: String]? = nil) throws -> Launched {
        // Unique per spawn: a fixed name would let a second gooseagent (or a second start)
        // truncate a log file the live daemon still holds open at a non-zero offset.
        let logURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("gooseagent-server-\(getpid())-\(UUID().uuidString.prefix(8)).log")
        FileManager.default.createFile(atPath: logURL.path, contents: nil)
        guard let log = try? FileHandle(forWritingTo: logURL) else {
            throw GooseAgentError.connectionFailed("could not open \(logURL.path)")
        }
        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: binary)
        proc.arguments = ["server"]
        // Same PATH the lookup walked, plus the binary's directory last: gooseagent
        ...
        do {
            ...
            throw GooseAgentError.connectionFailed("gooseagent server spawn: \(error.localizedDescription)")
        }
    ...
    ///
    /// gooseagent picks the shell of every pane from `$SHELL`, and launchd does not export it: a GUI
    /// app started from Finder has no `SHELL` in its environment (`launchctl getenv SHELL` is
    /// empty), so a server spawned from gooseagent would give the user `sh` panes instead of their
    ...
    /// deliberate choice. When the login shell cannot be resolved, nothing is set and gooseagent
    /// keeps whatever default it has.
    /// Process identity of the GUI app, which must not reach the daemon: the server outlives
    /// gooseagent and every pane inherits its environment, so `__CFBundleIdentifier` would make a
    /// pane's `defaults`/NSUserDefaults hit gooseagent's domain and TCC prompts get attributed to
    /// the app (revoking gooseagent's permissions would then break agents). `DYLD_*` and friends
