...
public actor DeviceFileService {
    ...
    public func uploadFile(
        ...
        guard localURL.isFileURL, values.isRegularFile == true else {
            throw GooseAgentError.fileTransferFailed("only regular local files can be uploaded")
        }
        ...
        case .ssh:
            ...
            let temporary = destination + ".gooseagent-\(UUID().uuidString.lowercased()).part"
            let installCommand = conflictPolicy == .replace
                ? "mv -f \"$tmp\" \"$dest\""
                : "ln \"$tmp\" \"$dest\" && rm -f \"$tmp\""
            let command = """
            umask 077
            tmp=\(GooseService.shellQuoted(temporary))
            dest=\(GooseService.shellQuoted(destination))
    ...
    public func downloadFile(
        ...
        guard localDirectory.isFileURL else {
            throw GooseAgentError.fileTransferFailed("a local destination directory is required")
        }
        ...
        case .ssh:
            let totalOutput = try await runSSHData(
                command: "wc -c < \(GooseService.shellQuoted(source))",
                timeout: 15
            )
            guard let totalBytes = Int64(
                ...
                throw GooseAgentError.fileTransferFailed("the remote host returned an invalid file size")
            }
            let temporary = destination
                .deletingLastPathComponent()
                .appendingPathComponent(
                    ".\(destination.lastPathComponent).gooseagent-\(UUID().uuidString.lowercased()).part"
                )
            defer { try? FileManager.default.removeItem(at: temporary) }
            try await runSSHDownload(
                command: "cat \(GooseService.shellQuoted(source))",
    ...
    private func absolutePath(_ requestedPath: String) async throws -> String {
        ...
        guard expanded.hasPrefix("/") else {
            throw GooseAgentError.fileOperationFailed("path must be absolute or start with ~")
        }
    ...
    private func homeDirectory() async throws -> String {
        ...
        case .ssh:
            ...
            guard home.hasPrefix("/") else {
                throw GooseAgentError.fileOperationFailed("could not resolve the remote home directory")
            }
    ...
    private func resolvedDestination(
        ...
        case .fail:
            guard !exists else {
                throw GooseAgentError.fileTransferFailed("“\(name)” already exists")
            }
            return original
        case .keepBoth:
            ...
            throw GooseAgentError.fileTransferFailed("could not find an available name for “\(name)”")
        }
    }
    ...
    private func pathExists(_ path: String) async throws -> Bool {
        ...
        case .ssh:
            let command = """
            if [ -e \(GooseService.shellQuoted(path)) ] || [ -L \(GooseService.shellQuoted(path)) ]; then
                printf 1
            else
                printf 0
            fi
    ...
    private func runSSHData(command: String, timeout: TimeInterval) async throws -> Data {
        guard case .ssh(let target) = device.kind else {
            throw GooseAgentError.fileOperationFailed("SSH is unavailable for a local device")
        }
    ...
    private func runSSHUpload(
        ...
        guard case .ssh(let target) = device.kind else {
            throw GooseAgentError.fileTransferFailed("SSH is unavailable for a local device")
        }
        let authentication = SSHTunnel.authenticationConfiguration(for: device.id)
        defer { authentication.discardAuthorization() }
        let arguments = authentication.arguments + [
            ...
            "-o", "ServerAliveCountMax=3",
            SSHTunnel.sshDestination(target),
            "exec /bin/sh -c \(GooseService.shellQuoted(command))",
        ]
    ...
    static func parseRemoteListing(_ data: Data, directory: String) throws -> [DeviceFileEntry] {
        ...
        guard payload.count.isMultiple(of: 4) else {
            throw GooseAgentError.fileOperationFailed("the remote host returned malformed directory data")
        }
    ...
    static func remoteListingCommand(path: String) -> String {
        """
        cd \(GooseService.shellQuoted(path)) || exit 1
    ...
    private static func copyLocalFile(
        ...
        guard values.isRegularFile == true else {
            throw GooseAgentError.fileTransferFailed("only regular files can be transferred")
        }
        let totalBytes = Int64(values.fileSize ?? 0)
        let temporary = destination
            .deletingLastPathComponent()
    ...
    private static func installDownloadedFile(
        ...
        if manager.fileExists(atPath: destination.path) {
            guard replace else {
                throw GooseAgentError.fileTransferFailed("“\(destination.lastPathComponent)” already exists")
            }
            ...
            try await withCheckedThrowingContinuation {
                ...
                DispatchQueue.global(qos: .userInitiated).async {
                    ...
                    do {
                        ...
                        continuation.resume(
                            throwing: GooseAgentError.fileOperationFailed(
                    ...
                    if processBox.didTimeOut {
                        continuation.resume(
                            throwing: GooseAgentError.fileOperationFailed("SSH timed out")
                        )
            ...
            try await withCheckedThrowingContinuation {
                ...
                DispatchQueue.global(qos: .userInitiated).async {
                    ...
                    do {
                        ...
                        continuation.resume(
                            throwing: GooseAgentError.fileTransferFailed(
                                "could not start SSH: \(error.localizedDescription)"
                            )
                        )
                        return
