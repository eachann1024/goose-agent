...
/// A machine running gooseagent. `local` talks straight to the Unix socket;
/// `ssh` reaches the remote socket through an OpenSSH stream-local forward;
/// `tailcat` reaches a socket exposed by the gooseagent.tailcat server plugin
...
public struct Device: Codable, Sendable, Identifiable, Equatable, Hashable {
    ...
    /// Socket path override; nil means the default session socket (~/.config/gooseagent/gooseagent.sock).
    ...
    public var subtitle: String {
        switch kind {
        case .local: return isNamedSession ? "This Mac · session \(name)" : "This Mac · gooseagent.sock"
...
public final class DeviceStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "dev.eachann.gooseagent.devices")

    public init(directory: URL? = nil) {
        let base = directory ?? FileManager.default
            .urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("GooseAgent", isDirectory: true)
